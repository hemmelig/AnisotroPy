# -*- coding: utf-8 -*-
"""
Modelling the effective elastic properties of a material consisting of an isotropic host
matrix with unidirectionally aligned isotropic spheroid inclusions.

For more information, please read:

    Tandon, G.P. and Weng, G.J., 1984. The effect of aspect ratio of inclusions on the
    elastic properties of unidirectionally aligned composites. Polymer composites, 5(4),
    pp.327-333.
...
def model(matrix, inclusions, alpha, fraction_inc):
    """
    Calculate the averaged elastic parameters as described in Tandon and Weng (1984),
    hereafter TW84.

    Their theory describes the effective elastic properties of a medium consisting of an
    isotropic host matrix with unidirectionally aligned isotropic spheroid inclusions.
    ...
    s11 = (a2 + (3.0 * a3 - 1.0) / a4 - (a2 + (3.0 * a3) / a4) * g) / (2.0 * a1)
    s22 = s33 = (3.0 * a3) / (8.0 * a1 * a4) + ((a2 - 9.0 / (4.0 * a4)) * g) / (
        4.0 * a1
    )
    s23 = s32 = (a3 / (2.0 * a4) - (a2 + 3.0 / (4.0 * a4)) * g) / (4.0 * a1)
    s21 = s31 = (-a3) / (2.0 * a1 * a4) + (3.0 * a3 / a4 - a2) * g / (4.0 * a1)
    s12 = s13 = (a2 + 1.0 / a4) / (-2.0 * a1) + (a2 + 3.0 / (2.0 * a4)) * g / (
        2.0 * a1
    )
    s44 = (a3 / (2.0 * a4) + (a2 - 3.0 / (4.0 * a4)) * g) / (4.0 * a1)
    s55 = s66 = (a2 - (a3 + 1.0) / a4 - 0.5 * (a2 - (3.0 * (a3 + 1.0)) / a4) * g) / (
        4.0 * a1
    )

    # B term from TW84 (after equation 17)
    B1 = fraction_inc * D1 + D2 + (1.0 - fraction_inc) * (D1 * s11 + 2.0 * s21)
    B2 = fraction_inc + D3 + (1.0 - fraction_inc) * (D1 * s12 + s22 + s23)
    B3 = fraction_inc + D3 + (1.0 - fraction_inc) * (s11 + (1.0 + D1) * s21)
    B4 = fraction_inc * D1 + D2 + (1.0 - fraction_inc) * (s12 + D1 * s22 + s23)
    B5 = fraction_inc + D3 + (1.0 - fraction_inc) * (s12 + s22 + D1 * s23)
    ...
    E11 = E0 / (1.0 + fraction_inc * (A1 + 2.0 * nu * A2) / A)

    # Transverse Young's modulus E22
    E22 = E0 / (
        1.0
        + fraction_inc
        * (-2.0 * nu * A3 + (1.0 - nu) * A4 + (1.0 + nu) * A5 * A)
        / (2.0 * A)
    )

    # In-plane shear modulus mu12
    mu12 = 1.0 + fraction_inc / ((mu / (mui - mu)) + 2.0 * (1.0 - fraction_inc) * s66)
    mu12 *= mu

    # Out-plane shear modulus mu23
    mu23 = 1.0 + fraction_inc / ((mu / (mui - mu)) + 2.0 * (1.0 - fraction_inc) * s44)
    mu23 *= mu

    # Equation for nu31 (Sayers, 1992 (36))
    nu31 = nu - (
        fraction_inc
        * (nu * (A1 + 2.0 * nu * A2) + (A3 - nu * A4))
        / (A + fraction_inc * (A1 + 2.0 * nu * A2))
    )
    ...
    denom = (
        ...
        - nu * (1.0 + 2.0 * nu31)
        + fraction_inc
        * (2.0 * (nu31 - nu) * A3 + (1.0 - nu * (1.0 + 2.0 * nu31)) * A4)
        / A
    )
    K0 = la + mu  # Plane-strain bulk modulus of matrix
    K23 = K0 * numer / denom
    nu12_sq = E11 / E22 - (1.0 / mu23 + 1.0 / K23) * E11 / 4.0

    # Calculate C per Sayers (1992) equations (24)--(29), rotated into a frame
    # in which the inclusions are orientated vertically
    C = np.array(
        [
            [E11 + 4.0 * nu12_sq * K23, 2.0 * nu31 * K23, 2.0 * nu31 * K23, 0, 0, 0],
            [2.0 * nu31 * K23, mu23 + K23, -mu23 + K23, 0, 0, 0],
            [2.0 * nu31 * K23, -mu23 + K23, mu23 + K23, 0, 0, 0],
            [0, 0, 0, mu23, 0, 0],
            [0, 0, 0, 0, mu12, 0],
            [0, 0, 0, 0, 0, mu12],
